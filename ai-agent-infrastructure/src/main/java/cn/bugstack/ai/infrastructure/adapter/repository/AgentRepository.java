...
import domain.agent.model.valobj.*;
...
import static domain.agent.model.valobj.Enums.AiAgentEnumVO.*;
...
public class AgentRepository implements IAgentRepository {
    ...
    /**
     * 场景1：根据客户端ID，顺藤摸瓜找到它最底层的 API 配置 (URL、密钥)
     * 这是一个经典的“一对多、多级关联”查询
     */
    @Override
    ...
    public List<AiClientAdvisorVO> AiClientAdvisorVOByClientIds(List<String> clientIdList) {
        ...
        for (String clientId : clientIdList) {
            ...
            for (AiClientConfig config : configs) {
                ...
                AiClientAdvisorVO.VectorStoreMemory vectorStoreMemory=null;
                ...
                if (extParam != null && !extParam.trim().isEmpty()) {
                    try {
                        ...
                        }else if ("VectorStoreChatMemoryAdvisor".equals(aiClientAdvisor.getAdvisorType())) {
                            // 🌟 解析长期记忆配置
                            vectorStoreMemory = JSON.parseObject(extParam, AiClientAdvisorVO.VectorStoreMemory.class);
                        }
                    } catch (Exception e) {
                        // 解析失败时忽略，使用默认值null
                    }
                ...
                AiClientAdvisorVO advisorVO = AiClientAdvisorVO.builder()
                        ...
                        .ragAnswer(ragAnswer)
                        .vectorStoreMemory(vectorStoreMemory)
                        .build();
